...
typedef void (*BikeMode_t)(void);


BikeMode_t drive_modes[] = {
    drive_eco,      
    drive_normal,   
    drive_sport     
};
...
void crash_simulation(void) {
    // HỌC VIÊN BẮT ĐẦU VIẾT CODE TỪ ĐÂY
    volatile uint8_t dummy_payload[100];
    crash_simulation();
    // HỌC VIÊN KẾT THÚC VIẾT CODE
}
...
int main() {
    ...
    // HỌC VIÊN BẮT ĐẦU VIẾT CODE TỪ ĐÂY
int ASSIST_LEVEL = 2; 
if (ASSIST_LEVEL >= 0 && ASSIST_LEVEL < 3) {
    drive_modes[ASSIST_LEVEL](); 
} else {
    printf("Loi: ASSIST_LEVEL khong hop le!\n");
}
    // HỌC VIÊN KẾT THÚC VIẾT CODE
